...
async def pm_next_page(bot, query):
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {replace_username(file.file_name)}", callback_data=f'pmfile#{file.file_id}')] for file in files ]
        else:
            btn = [[InlineKeyboardButton(text=f"{replace_username(file.file_name)}", callback_data=f'pmfile#{file.file_id}'),
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {replace_username(file.file_name)}", callback_data=f'{pre}#{file.file_id}')] for file in files ]
        else:
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text="⌧ 𝐍𝐎 𝐌𝐎𝐑𝐄 ⌧", callback_data="pages")]
        )
    if PM_IMDB:
...
async def pm_spoll_choker(msg):
    ...
    try:
        ...
        logger.exception(e)
        k = await msg.reply("I couldn't find any movie in that name. Check your spelling in Google then try again and Read instruction ")
        await asyncio.sleep(8)
        await k.delete()
        return
    movielist = []
    if not movies:
        k = await msg.reply("I couldn't find anything related to that. Check your spelling in Google then try again and  Read instructions")
        await asyncio.sleep(8)
        await k.delete()
        return        
